function [dcor,pVal] = distcorr(x,y)
...
% ADDED BY ME:
% This part I added (Noga) to get a bootstrap p-val

if nargout>1
    REPEATS = 10000;
    dcorDist = nan(1,REPEATS);
    for i=1:REPEATS
        xPerm = permVec(x);
        dcorDist(i) = distcorr(xPerm,y);
    end

    assert(all(dcorDist>0))
    pVal = mean(dcorDist>dcor);
end